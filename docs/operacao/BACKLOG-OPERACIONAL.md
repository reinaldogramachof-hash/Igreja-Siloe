...
| TEN-01 | Organizações, vínculos e isolamento entre igrejas | 1 | 4 | Codex (exceção DEC-043) + Antigravity (tela) | Arquiteto | 1 | Revisao | migração + RLS + resolução real de papel **entregues e revisadas** (código); falta aplicar a migração no Supabase real e rodar `ten_01_isolation.sql` de fato (Codex tem escrita, Arquiteto só leitura) | TEN-01-ESPECIFICACAO.md · branch tarefa/TEN-01 (commit `330a77b`) |
