...
| OPS-01 | Homologar HostGator Plano M + frontend estático + Supabase | 1 | 0 | Arquiteto + Dev Backend | Arquiteto + Dev Frontend | 1 | Execucao | subdomínio + SSL ok; migração `homolog` **aplicada e validada** (`supabase/validate/` — 14/14 + 6/6 PASS). Falta: `homolog` em Exposed schemas · deploy `homolog-echo` · `next.config.ts` export + `.htaccess` + runbook · prova A/B frontend | OT-OPS-01 · branch tarefa/OPS-01-homologacao |
