...
| SEC-01 | Auth, recuperação, convite e logout reais | 1 | 1 | Arquiteto + Dev Backend | Arquiteto | 1 | **Bloqueada** | revisão de segurança do Arquiteto feita em 2026-09-10 — achou 1 crítico (papel escolhido no cliente, sem verificação de servidor — §17.1), 1 alto (open redirect em `/auth/callback`), 2 médios (cadastro simulado; marca Siloé residual), 1 baixo. Codex corrige antes de seguir | branch tarefa/SEC-01-auth (commit `04e32d1`) |
