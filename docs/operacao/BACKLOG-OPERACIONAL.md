...
| OPS-01 | Homologar HostGator Plano M + frontend estático + Supabase | 1 | 0 | Arquiteto + Dev Backend | Arquiteto + Dev Frontend | 1 | Execucao | branch `tarefa/OPS-01-homologacao`; subdomínio criado + SSL ok; migração `supabase/migrations/20260908183000_homolog.sql` + rollback + `homolog-echo` versionados, aguardando Reinaldo aplicar no SQL Editor. Falta: `next.config.ts` export + `.htaccess` + runbook + prova A/B | OT-OPS-01 · branch tarefa/OPS-01-homologacao |
