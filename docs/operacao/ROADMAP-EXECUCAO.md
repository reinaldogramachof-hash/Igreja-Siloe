...
|---|---|---|---|
| **A — Base limpa** | 0 + início da 1 | ~~PLAN-12~~, ~~SIL-01~~ **cancelados (DEC-026 — Siloé migrou para outro projeto)**; TAREFA-001 (escopo reduzido a dado fictício remanescente); OPS-01 (prova HostGator Plano M + Supabase); NEG-01 (preços, parcial); TAREFA-002/Landing Page **adiantada e já commitada** (DEC-025) | Exportação estática provada no Plano M; repo sem marca Siloé; Landing Page publicável assim que NEG-01 fechar |
...
| **D — Isolamento multi-tenant + console** | 4 (antecipada) | TEN-01 (organizações, vínculos, RLS, todo registro pertence a uma org) — **desenho já considera os requisitos do ADM-01 expandido (DEC-027/DEC-028): cliente/tenant, licença e módulo contratado fazem parte do modelo desde o início, para não retrabalhar**; ADM-01 kickoff (spec + modelo de dados) | Casos §8.3: igreja A não lê/edita/exporta dados da B; metadados do cliente não conferem privilégio; modelo de licença/módulo não exige retrabalho na Fase F |
...
| **F — Fundação comercial** | 4 | ADM-01 conclusão (vendas, licenças, clientes, módulos, MFA — escopo DEC-027); FAT-01 (webhooks idempotentes, atraso/cancelamento/reativação); PUB-01 (página pública por tenant) | Admin de igreja não acessa console/API; webhook falso rejeitado; evento repetido idempotente; falha de pagamento não apaga igreja |
| **G — MVP** | 3 + 5 | ~~SIL-02~~ **cancelado (DEC-026)**; empacotar Essencial Lite; dimensionar limites (NEG-01) | MVP sem falha crítica; materiais só descrevem o entregue |
